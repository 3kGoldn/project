...
# 创建统一保存所有视角图的目录
all_views_dir = "/content/InstantMesh/outputs/all_views"
os.makedirs(all_views_dir, exist_ok=True)

# 定义视角名称顺序 (注意: 根据您的视图顺序可能需要调整)
view_orders = {
    'main': ['front', 'front_right', 'right', 'back', 'left', 'front_left'],
    'side': ['front', 'front_right', 'right', 'back', 'left', 'front_left'],
    'back': ['front', 'front_right', 'right', 'back', 'left', 'front_left']
}

# 处理3个视角组的图片
for view_type, composite in zip(['main', 'side', 'back'], [composite_main, composite_side, composite_back]):
    # 将拼接图转换为数组
    composite_array = np.array(composite)

    # 计算每张图片的尺寸 (假设是2行3列的布局)
    height = composite_array.shape[0] // 2
    width = composite_array.shape[1] // 3

    # 切割图片
    for row in range(2):
        for col in range(3):
            idx = row * 3 + col
            img = composite_array[row*height:(row+1)*height, col*width:(col+1)*width]

            # 生成文件名 (如: main_front.png, side_right.png等)
            view_name = view_orders[view_type][idx]
            filename = f"{view_type}_{view_name}.png"

            # 保存图片
            Image.fromarray(img).save(os.path.join(all_views_dir, filename))

print(f"\n✅ 所有18张视角图已统一保存到 {all_views_dir}")
print("文件命名格式: [视图类型]_[视角名称].png")
print("例如: main_front.png, side_right.png, back_back.png 等")
